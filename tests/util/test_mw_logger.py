...
def test_get():
    ...
    assert logger.name == mw_logger.__get_mw_logger_name()


def test_get_mw_logger_name():
    """Test to lock down the name of the logger used."""
    # Okay to use hidden API for testing only.
    assert "MATLABProxyApp" == mw_logger.__get_mw_logger_name()
...
def test_get_with_no_environment_variables(monkeypatch):
    ...
    env_names_list = mw_logger.get_environment_variable_names()
