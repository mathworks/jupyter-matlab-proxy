...
def matlab_process_setup_fixture():
    ...
    devel_file = Path(
        os.path.join(os.getcwd(), 'jupyter_matlab_proxy', 'devel.py'))
    ...
    matlab_cmd = [
        python_executable, "-u",
        str(devel_file), "matlab", "--ready-file",
        str(matlab_ready_file)
    ]
    variables = matlab_setup_variables(devel_file, matlab_cmd,
                                       matlab_ready_file, master, slave)

    return variables


@pytest.fixture(name='matlab_process_valid_nlm')
def matlab_process_valid_nlm_fixture(matlab_port_setup, matlab_process_setup,
                                     valid_nlm):
    ...
    matlab_process = subprocess.Popen(matlab_process_setup.matlab_cmd,
...
def matlab_process_invalid_nlm_fixture(matlab_port_setup, matlab_process_setup,
                                       invalid_nlm):
...
def xvfb_process_setup_fixture():
    ...
    xvfb_setup_variables = namedtuple('xvfb_setup_variable',
                                      ['xvfb_cmd', 'xvfb_ready_file'])
    ...
    xvfb_cmd = [
        python_executable, "-u",
        str(devel_file), "xvfb", "--ready-file",
        str(xvfb_ready_file)
    ]
